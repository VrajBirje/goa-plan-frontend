  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      ...
      try {
        ...
        if (data.image_url) {
          setUploadedImages([data.image_url]); // Set the existing image URL if available
        }
      } catch (error) {
  ...
  const handleUploadSuccess = (result: any) => {
    ...
    if (info && typeof info === "object" && "secure_url" in info) {
      ...
      setUploadedImages((prevImages) => {
        const updatedImages = [image_url, ...prevImages]; // Update the images with the new one
        // Set the first uploaded image to imageUrl in formData
        setFormData((prevData) => ({
          ...prevData,
          image_url: updatedImages[0], // Set the first image to formData.image_url
        }));
